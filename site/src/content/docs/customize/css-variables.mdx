...
The original 5.3.8 declarations remain earlier in the compiled stylesheet for source compatibility. The reactive declaration pass targets browsers with CSS custom properties, `color-mix()`, relative colors, and native CSS math; it does not depend on `contrast-color()` or a feature-query wrapper. In that path, `--bs-*-rgb` properties contain colors rather than comma-separated channels. Apply alpha with relative color syntax:
