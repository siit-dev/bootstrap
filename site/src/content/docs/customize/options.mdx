...
| `$enable-reactive-colors`      | `true` (default) or `false`        | Derives the supported light-mode color states from CSS source variables with `color-mix()`. |
| `$enable-reactive-spacing`     | `true` (default) or `false`        | Derives Bootstrap's default spacing and gutter utility values from `--bs-spacer`. Explicit custom `$spacers` maps remain static. |
| `$enable-reactive-color-fallbacks` | `true` or `false` (default)   | Emits Sass-compiled color declarations before feature-queried reactive overrides for browsers without `color-mix()`. |
